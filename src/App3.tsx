...
import { useNetwork, useAccount, useConnect, useDisconnect, usePublicClient } from "wagmi";
import "./App.css";
...
const web3ankr = new Web3(new Web3.providers.HttpProvider(`${ANKR_RPC_URL}/${ANKR_API_KEY}`))


// Web3 ve sözleşme instance'larını başlatıyoruz
const contractInstance = new web3ankr.eth.Contract(contractABI, contractAddress);
function App() {
...
useEffect(() => {
  if (walletAddress && contractInstance) {
    ...
    checkSubscription();
    updateSubscriptionFee(contractInstance);
  }
...
if (Array.isArray(ethereum.providers)) {
  ...
  provider = ethereum;
      ...
      if (!provider || !provider.isMetaMask) {
        console.error("MetaMask not found or another wallet extension is active");
        alert("MetaMask is not installed or active. Please install MetaMask or switch to MetaMask if another wallet is active.");
        return null;
      }
...
if (Array.isArray(ethereum.providers)) {
  ...
  const checkSubscription = async () => {
    try {
      ...
      const response = await fetch(`https://predatoraibot.com:5000/check_subscription?address=${walletAddress}`);
 ...
 const handleSubscribe = async () => {
  if (!isConnected) {
    ...
    try {
      await connect({ connector: connectors[0] });
       ...
       if (walletType === 'MetaMask') {
        window.location.href = 'https://metamask.app.link/dapp/predatoraibot.com';
      } else if (walletType === 'TrustWallet') {
        window.location.href = 'https://link.trustwallet.com/open_url?coin_id=60&url=https://predatoraibot.com';
      } else {
  ...
  if (walletAddress && contractInstance) {
    try {
      ...
      alert('Subscription process failed. Redirecting to your wallet dApp browser.');

      // Kullanıcının cüzdan türüne göre yönlendirme yapalım
      if (walletType === 'MetaMask') {
        window.location.href = 'https://metamask.app.link/dapp/predatoraibot.com';
      } else if (walletType === 'TrustWallet') {
        window.location.href = 'https://link.trustwallet.com/open_url?coin_id=60&url=https://predatoraibot.com';
  ...
  return (
    ...
    <div className="App">
     {isLoading ? <Preloader /> : <Navbar />} {/* Preloader yüklenme durumu true iken gösterilir */}
      <VideoNews />
  ...
  <div className="card">
    ...
    <div className="card-body">
      <select id="walletType" value={walletType} onChange={handleWalletTypeChange} className="wallet-select">
        ...
        <option className="wallet-option" value="Coinbase">Others</option>
      </select>
    </div>
    {isConnected && (
      <div className="card-body">
